...
intrinsic '-' (A::GrpGL2HatElt) -> GrpGL2HatElt
...
intrinsic '*' (A::GrpGL2HatElt,B::GrpGL2HatElt) -> GrpGL2HatElt
    {"} // "
    GA := Parent(A);
    GB := Parent(B);
...
intrinsic '*' (A::GrpGL2HatElt,z::FldComElt) -> FldComElt
   {"} // "
...
intrinsic '*' (A::GrpGL2HatElt,z::RngIntElt) -> FldRatElt
   {"} // "
   // Action on the upper half plane:
   a,b,c,d := Explode(Eltseq(A`Element));
    if (c*z + d) ne 0 then 
...
intrinsic '*' (A::GrpGL2HatElt,z::Infty) -> FldRatElt
   {"} // "
   // Action on the upper half plane:
   print 4;
...
intrinsic '*' (A::GrpGL2HatElt,z::FldRatElt) -> FldRatElt
   {"} // "
   // Action on the upper half plane:
   a,b,c,d := Explode(Eltseq(A`Element));
    if (c*z + d) ne 0 then 
...
intrinsic '*' (A::GrpGL2HatElt,z::[FldRatElt]) -> SeqEnum
   {"} // "
   // Action on elements of upper half plane union cusps:
...
intrinsic '^' (A::GrpGL2HatElt,k::RngIntElt) -> GrpGL2HatElt
   {"} // "
